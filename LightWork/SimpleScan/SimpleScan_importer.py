...
def modified_z_score(intensity):
    ...
    mad_int = np.median([np.abs(intensity - median_int)]) # Mean absolute deviation
    modified_z_scores = 0.6745 * (intensity - median_int) / mad_int #0.75th quartile of normal distribution
...
def RemoveCosmicRays(spec, m, threshold):
    ...
    for idx in spike_indices:
        ...
        if idx+2+m > len(spec):
            ...
            w = np.arange(idx-m,idx+1+m) # we select 2 m + 1 points around our spike
...
def Efield(vtop, vbot, ds, eps_sample): # using Ouri
    efield = 1e3*(vtop-vbot)/((dbot+dtop)*eps_sample/eps_bn+ ds)  # convert to mv/nm
    return efield
    
def n(vtop, vbot):
    n = -eps_bn*8.85418e-12/(1.602e-19)*(vtop/(dtop*1e-9) + vbot/(dbot*1e-9))*1e-4 # convert to cm^-2
    return(n)
...
def plot_sorter_linecut(master_data, map_to_sorter, sorter_cuts_to_plot=[], title ='', cr_m=3, cr_thresholds=[], sc_e_min=None, sc_e_max=None, z_min=None, z_max=None):
    ...
    if sorter_cuts_to_plot:
        true_ns = [list(sorter)[(abs(sorter-val)).argmin()] for val in sorter_cuts_to_plot]
    ...
    contourplot = axs[0].contourf(energies, sorter, spectra, levels =np.linspace(zmin, zmax, 500), cmap = 'Greys')
...
def plot_energy_linecut(master_data, map_to_sorter, title='', energy_cuts_to_plot=[], halfwidth=3, cr_m=3, cr_thresholds=[], sc_e_min=None, sc_e_max=None, z_min=None, z_max=None):
    ...
    if energy_cuts_to_plot:
        true_energies = [energies[(abs(energies-val)).argmin()] for val in energy_cuts_to_plot]

    fig, axs = plt.subplots(2,1)
    for count, d in enumerate(true_energies):
        idx = list(energies).index(d)
        axs[1].plot(sorter, np.mean(spectra[:,idx-halfwidth:idx+halfwidth], axis=1), label = '{} eV'.format(np.round(d, 2)), marker = '.')
        axs[0].axvspan(energies[idx-halfwidth], energies[idx+halfwidth], alpha=0.5, color='C{}'.format(count))
    plt.xlabel('Energy [eV]')
    axs[1].set_ylabel('Intensity [a.u.]')
    axs[1].legend(prop={'size': 10});
    zmin = z_min if z_min is not None else np.min(spectra)
...
file = open(r'C:\Users\heinz\Documents\User Files\aidan\simpleSweeptest\Jun-09-2022\test.pkl', 'rb')
...
sample['master_data'] = dict(sorted(sample['master_data'].items(), key = lambda x: x[1]['keithley_BG']['voltage [V]']))
...
plot_sorter_linecut(master_data = sample['master_data'], map_to_sorter = ['keithley_BG','voltage [V]'], title='test scan', sorter_cuts_to_plot=[0])
